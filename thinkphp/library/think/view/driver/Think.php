...
//                    $template = str_replace('.', DS, $controller) . $depr . (1 == $this->config['auto_rule'] ? Loader::parseName($request->action(true)) : $request->action());
                    $action = (1 == $this->config['auto_rule'] ? Loader::parseName($request->action(true)) : $request->action());
                    switch ($action){
                        case "index":
                            $template = 'common'.$depr.'index';
                            break;
                        case "add":
                            $template = 'common'.$depr.'add';
                            break;
                        case "edit":
                            $template = 'common'.$depr.'edit';
                            break;
                        default:
                            $template = str_replace('.', DS, $controller) . $depr . (1 == $this->config['auto_rule'] ? Loader::parseName($request->action(true)) : $request->action());
                            break;
